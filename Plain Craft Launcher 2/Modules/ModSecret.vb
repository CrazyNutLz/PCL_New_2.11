...
#Region "主题"
    ...
    Public Color9 As New MyColor(128, Color7)
    ...
    Private J4q As Boolean = False
    Private d4s As Int32 = 1

    Public Sub ThemeRefresh(Optional NewTheme As Integer = -1)
        Try
            If ModSecret.ThemeNow <> NewTheme OrElse NewTheme < 0 Then
                If NewTheme >= 0 Then
                    ModSecret.ThemeNow = NewTheme
                End If
                Dim themeNow As Integer = ModSecret.ThemeNow
                Select Case themeNow
                    ...
                    Case 2
                        ...
                        ColorSat = 50
                        ColorLightAdjust = 0
                        ColorHueTopbarDelta = {5, -5, 5}
                    Case 3
                        ColorHue = 60
                        ColorSat = 65
                        ColorLightAdjust = 0
                        ColorHueTopbarDelta = {-5, 10, -5}
                    Case 4
                        ColorHue = 20
                        ColorSat = 50
                        ColorLightAdjust = -5
                    ...
                    Case 12
                        If Not J4q Then
                            J4q = True
                            ColorHue = ModBase.RandomInteger(0, 359)
                            d4s = (ModBase.RandomInteger(0, 1) * 2 - 1) * 3
                        End If
                        ColorHue = (ColorHue + d4s) Mod 360
                        ColorSat = 65
                        ColorLightAdjust = 0
                    Case 13
                        ColorHue = 325
                        ColorSat = 90
                        ColorLightAdjust = 0
                        ColorHueTopbarDelta = {-10, 0, 15}
                    Case 14
                        ColorHue = Convert.ToInt32(ModBase.Setup.Get("UiLauncherHue", Nothing))
                        ColorSat = Convert.ToInt32(ModBase.Setup.Get("UiLauncherSat", Nothing))
                        ColorLightAdjust = Convert.ToInt32(Operators.SubtractObject(ModBase.Setup.Get("UiLauncherLight", Nothing), 20))
                        ColorHueTopbarDelta = Operators.SubtractObject(ModBase.Setup.Get("UiLauncherDelta", Nothing), 90)
                    Case Else
                        If themeNow <> 42 Then
                            ColorHue = 215
                            ColorSat = 90
                        ...
                        End If
                End Select
                Color1 = New ModBase.MyColor().FromHSL2(CDbl(ColorHue), CDbl(ColorSat) * 0.2, 25.0 + CDbl(ColorLightAdjust) * 0.3)
                Color2 = New ModBase.MyColor().FromHSL2(CDbl(ColorHue), CDbl(ColorSat), CDbl(45 + ColorLightAdjust))
                Color3 = New ModBase.MyColor().FromHSL2(CDbl(ColorHue), CDbl(ColorSat), CDbl(55 + ColorLightAdjust))
                Color4 = New ModBase.MyColor().FromHSL2(CDbl(ColorHue), CDbl(ColorSat), CDbl(65 + ColorLightAdjust))
                Color5 = New ModBase.MyColor().FromHSL2(CDbl(ColorHue), CDbl(ColorSat), 80.0 + CDbl(ColorLightAdjust) * 0.4)
                Color6 = New ModBase.MyColor().FromHSL2(CDbl(ColorHue), CDbl(ColorSat), 91.0 + CDbl(ColorLightAdjust) * 0.1)
                Color7 = New ModBase.MyColor().FromHSL2(CDbl(ColorHue), CDbl(ColorSat), 94.0)
                Color8 = New ModBase.MyColor().FromHSL2(CDbl(ColorHue), CDbl(ColorSat), 97.0)
                Color9 = New ModBase.MyColor(128.0, Color7)
                ColorSemiTransparent = New ModBase.MyColor(1.0, Color8)
                ...
                Application.Current.Resources("ColorBrush9") = New SolidColorBrush(Color9)
                Application.Current.Resources("ColorObject1") = Color.FromArgb(Color1.A, Color1.R, Color1.G, Color1.B)
                Application.Current.Resources("ColorObject2") = Color.FromArgb(Color2.A, Color2.R, Color2.G, Color2.B)
                Application.Current.Resources("ColorObject3") = Color.FromArgb(Color3.A, Color3.R, Color3.G, Color3.B)
                Application.Current.Resources("ColorObject4") = Color.FromArgb(Color4.A, Color4.R, Color4.G, Color4.B)
                Application.Current.Resources("ColorObject5") = Color.FromArgb(Color5.A, Color5.R, Color5.G, Color5.B)
                Application.Current.Resources("ColorObject6") = Color.FromArgb(Color6.A, Color6.R, Color6.G, Color6.B)
                Application.Current.Resources("ColorObject7") = Color.FromArgb(Color7.A, Color7.R, Color7.G, Color7.B)
                Application.Current.Resources("ColorObject8") = Color.FromArgb(Color8.A, Color8.R, Color8.G, Color8.B)
                Application.Current.Resources("ColorObject9") = Color9
                ThemeRefreshMain()
                If themeNow <> 12 AndAlso themeNow <> 14 Then
                    ModBase.Log("[UI] 刷新主题：" & Conversions.ToString(themeNow), ModBase.LogLevel.Normal, "出现错误")
                End If
            End If
        Catch ex As Exception
    ...
    Public Sub ThemeRefreshMain()
        ...
        Sub()
            ...
            '加载图片（Source 为 String 时）
            If ModSecret.ThemeNow >= 5 AndAlso ModSecret.ThemeNow <> 14 Then
                Dim path = ModBase.PathImage & "Themes/" & CStr(ModSecret.ThemeNow) & ".png"
                ModMain.FrmMain.ImgTitle.Source = path
            Else
                ModMain.FrmMain.ImgTitle.Source = Nothing
            End If
